			...
			$.post( base_url + "/krs/getmk",$( "#form-susun-krs" ).serialize(), function( data ) 
			{
				if(data.status === 'OK')
				{
					...
					$.each(data.results, function(key, item) 
					{
						...
						code += '<label style="font-size:1em">\n';
						code += '<input type="checkbox" name="mk[]" value="'+item.course_id+'" onclick="set_krs('+item.course_id+')"> <span class="cr"><i class="cr-icon fa fa-check"></i></span>\n';
						code += '</label>\n';
						code += '</div>\n';
						code += '</td>\n';
		...
		$.get( base_url + "/krs/getmkupdate", function( data ) 
		{
			if(data.status === 'OK')
			{
				...
				$.each(data.results, function(key, item) 
				{
					code += '<tr>\n';
					code += '<td>\n';
					code += '<input type="radio" name="mk" class="radio-bigger" value="'+item.course_id+'" required>\n';
					code += '</td>\n';
					code += '<td>'+item.course_code+'</td>\n';
					code += '<td>'+item.course_name+'</td>\n';
					code += '<td>'+item.sks+'</td>\n';
			...
			if(data.status === 'OK')
			{
				$('#modal-add-mk').modal('show');
					...
					$.each(data.results, function(key, item) 
					{
						...
						code += '<div class="checkbox">\n';
						code += '<label style="font-size:1em">\n';
						code += '<input type="checkbox" name="mk[]" value="'+item.course_id+'" onclick="cek_sksnya('+item.course_id+')"> <span class="cr"><i class="cr-icon fa fa-check"></i></span>\n';
						code += '</label>\n';
						code += '</div>\n';
						code += '</td>\n';
						code += '<td>'+ item.course_code +'</td>\n';
