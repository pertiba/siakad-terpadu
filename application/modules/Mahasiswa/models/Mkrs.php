...
class Mkrs extends CI_Model {
	...
	public function getMkupdate($param = 0, $semester = 'ganjil')
	{
		...
		if($study_point->num_rows())
		{
			...
			foreach($this->getPlain($param )->result() as $row)
	...
	public function getPlain($param = 0)
	{
		$query = $this->db->query("SELECT plain_studies.*, course.* FROM plain_studies JOIN course ON plain_studies.course_id = course.course_id WHERE student_id = ?", $param);
		return $query;
	}
	...
	public function create()
	{
		...
		if(is_array($this->input->post('mk')))
		{
			foreach($this->input->post('mk') as $key => $value) 
			{
				...
				$krk[] = array(
					'student_id' => $this->session->has_userdata('account_id'),
					'course_id' => $value,
					'lecturer_id' => '0',
					'years' => $this->input->post('thn_ajaran'),
	...
	public function add()
	{
		$get = $this->getPlain($this->session->has_userdata('account_id') )->result();
